...
intros; case H; trivial with arith.
intro; absurd B; trivial with arith.
Qed.
...
intros; case H; trivial with arith.
intro; absurd A; trivial with arith.
Qed.
...
elim H2; split; auto with arith.
elim H4; auto with arith.

apply H1; elim H4; auto with arith.
Qed.
...
intros; case (le_lt_eq_dec p n) as [ | -> ]; auto with arith.
Qed.
...
intros; case H; intros; simpl; auto with arith.
Qed.

Lemma plus_pred : forall n m : nat, 0 < n -> pred n + m = pred (n + m).
intros; apply eq_add_S.
rewrite <- S_pred.
rewrite <- plus_S; rewrite <- S_pred; auto with arith.
...
intros; case H; intros; simpl; auto with arith.
apply eq_S; rewrite <- S_pred; auto with arith.
Qed.

Lemma Sminus_aSb : forall a b : nat, b < a -> S (a - S b) = a - b.
intros; rewrite minus_Sn_m; auto with arith.
Qed.
...
apply plus_lt_compat_r; auto with arith.
Qed.

Lemma lt_not_eq : forall n m : nat, n < m -> n <> m.
intros; red; intros.
rewrite H0 in H; absurd (m < m); auto with arith.
Qed.
...
intros; unfold un; rewrite <- plus_n_Sm; auto with arith.
Qed.
...
intros; rewrite minus_un; rewrite <- S_pred; auto with arith.
Qed.

Lemma plus_deux : forall n : nat, n + deux = S (S n).
intros; unfold deux; repeat rewrite <- plus_n_Sm; auto with arith.
Qed.

Lemma minus_deux : forall n : nat, n - deux = pred (pred n).
intros; case n; simpl; auto with arith.
intros; apply minus_un; auto with arith.
Qed.

Lemma SSminus_deux : forall n : nat, deux <= n -> S (S (n - deux)) = n.
intros; rewrite minus_deux; do 2 (rewrite <- S_pred; auto with arith).
Qed.

Lemma plus_trois : forall n : nat, n + trois = S (S (S n)).
intros; unfold trois; repeat rewrite <- plus_n_Sm; auto with arith.
Qed.

Lemma minus_trois : forall n : nat, n - trois = pred (pred (pred n)).
intros; case n; simpl; auto with arith.
intros; apply minus_deux; auto with arith.
Qed.

Lemma SSSminus_trois :
 forall n : nat, trois <= n -> S (S (S (n - trois))) = n.
intros; unfold trois; rewrite Sminus_aSb; auto with arith.
rewrite SSminus_deux; auto with arith.
Qed.

Lemma plus_quatre : forall n : nat, n + quatre = S (S (S (S n))).
intros; unfold quatre; repeat rewrite <- plus_n_Sm; auto with arith.
Qed.
...
intros; apply lt_trans with (m := un); auto with arith.
Qed.

Lemma le6_lt2 : forall n : nat, six <= n -> deux < n.
intros; apply lt_le_trans with (m := 6); auto with arith.
Qed.

Lemma lt_O_minus : forall n m : nat, m < n -> 0 < n - m.
intros; apply plus_lt_reg_l with (p := m).
rewrite plus_zero; rewrite le_plus_minus_r; auto with arith.
Qed.
...
intros; unfold double; apply lt_plus_trans; auto with arith.
Qed.

Lemma double_S : forall n : nat, double (S n) = S (S (double n)).
unfold double; intros.
rewrite <- plus_n_Sm; simpl; auto with arith.
Qed.

Lemma le_double : forall n m : nat, double n <= double m -> n <= m.
intros n m; case (le_lt_dec n m); intros; auto with arith.
absurd (double n <= double m); auto with arith.
apply lt_not_le.
...
intros n m; case (le_lt_dec n m); intros; auto with arith.
absurd (double n <= S (double m)); auto with arith.
apply lt_not_le.
unfold double; apply le_lt_trans with (m := n + m).
apply lt_le_S; apply plus_lt_compat_r; auto with arith.
...
intros n m; case (le_lt_dec n m); intros; auto with arith.
absurd (triple m < triple n); auto with arith.
unfold triple; repeat (apply lt_plus_plus; auto with arith).
Qed.

Lemma lt_triple : forall n m : nat, triple n < triple m -> n < m.
intros n m; case (le_lt_dec m n); intros; auto with arith.
absurd (triple m <= triple n); auto with arith.
unfold triple; repeat (apply plus_le_compat; auto with arith).
Qed.

Lemma triple_S : forall n : nat, triple (S n) = S (S (S (triple n))).
intros; unfold triple; repeat rewrite <- plus_n_Sm; auto with arith.
Qed.

Lemma le_n_triplem : forall n m : nat, n <= m -> n <= triple m.
intros; unfold triple; do 2 apply le_plus_trans; auto with arith.
Qed.
...
simple induction a.
apply reste_2_0 with (q := 0); auto with arith.
apply even_O.

clear a; intros.
elim H; intros.
apply reste_2_1 with (q := q); auto with arith.
apply odd_S; auto with arith.

apply reste_2_0 with (q := S q).
apply even_S; auto with arith.

rewrite double_S; auto with arith.
Defined.
...
simple induction n.
apply reste_3_0 with (q := 0); auto with arith.

clear n; intros; elim H; intros.
apply reste_3_1 with (q := q); auto with arith.

apply reste_3_2 with (q := q); auto with arith.

apply reste_3_0 with (q := S q); auto with arith.
rewrite triple_S; rewrite e; auto with arith.
Defined.
...
intros; absurd (2 = 0); auto with arith.
Qed.
...
intros n; unfold Unmod3, reste3, tiers; case (quotient3 n);
 auto with arith.
intros; absurd (0 = 1); auto with arith.

intros; absurd (2 = 1); auto with arith.
Qed.
...
intros n; unfold Deuxmod3, reste3, tiers; case (quotient3 n);
 auto with arith.
intros; absurd (0 = 2); auto with arith.

intros; absurd (1 = 2); auto with arith.
Qed.

Lemma plus_deuxtiers_untiers :
 forall n : nat, Omod3 n -> double (tiers n) + tiers n = n.
intros; unfold double; apply triple_tiers; auto with arith.
Qed.

Lemma Splus_deuxtiers_untiers :
 forall n : nat, Unmod3 n -> S (double (tiers n) + tiers n) = n.
intros; unfold double; apply Striple_tiers; auto with arith.
Qed.

Lemma SSplus_deuxtiers_untiers :
 forall n : nat, Deuxmod3 n -> S (S (double (tiers n) + tiers n)) = n.
intros; unfold double; apply SStriple_tiers; auto with arith.
Qed.
...
simpl; intros; absurd (0 < 0); auto with arith.
...
intros n; unfold tiers; case (quotient3 n); intros q He; rewrite He.
case q; unfold double, triple.
simpl; intros; absurd (trois < 0); auto with arith.

clear He q n; intros n; case n.
simpl; intros; absurd (trois < trois); auto with arith.

clear n; intros; rewrite <- (plus_un (S (S n) + S (S n)));
 apply plus_lt_compat_l; unfold un; auto with arith.

case q; unfold double, triple.
simpl; intros; absurd (deux < 0); auto with arith.
...
case q; unfold double, triple.
simpl; intros; absurd (un < 0); auto with arith.
do 2 apply lt_S_n; auto with arith.
...
intros; unfold double, tiers; case (quotient3 a); simpl;
 intros q He; rewrite He; unfold triple; auto with arith.
Qed.

Lemma le_troistiers_un : forall n : nat, triple (tiers n) <= n.
intros; unfold tiers; case (quotient3 n); intros q ->; auto with arith.
Qed.
...
apply lt_S_n; info_auto with arith.
do 2 apply lt_S_n; auto with arith.
Qed.
...
intros n; unfold Omod3, Unmod3, reste3; simpl;
 case (quotient3 n); simpl; auto with arith.
intros; absurd (2 = 0); auto with arith.
Qed.
...
intros; elim H.
unfold tiers, trois; auto with arith.

intros; apply le_trans with (m := tiers m); auto with arith.
apply le_tiers_Stiers.
Qed.

Lemma le_tiers_six : forall n : nat, six <= n -> deux <= tiers n.
intros; elim H.
unfold tiers, six; auto with arith.
...
clear n; intros; absurd (S (S (S n)) <= 2); auto with arith.
Qed.
