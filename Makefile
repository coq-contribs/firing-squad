...
fire: algo.ml fire.ml
	rm *.mli; (mv list.ml polyList.ml || true); (sed -e 's/ List/ PolyList/g' algo.ml > algo.ml.tmp; mv algo.ml.tmp algo.ml); ocamlopt -o fire unix.cmxa graphics.cmxa datatypes.ml specif.ml polyList.ml peano_dec.ml nmax.ml autom.ml algo.ml fire.ml
