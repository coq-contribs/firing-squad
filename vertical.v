...
intros t x long H; elim H; clear H; intros H H0 H1;
 apply (Rec3 _ _ _ _ (make_deux_end t x)); auto with arith.
apply (GC_dollarB t x); auto with arith.
...
apply (Rec3 _ _ _ _ (make_quatre_end t x)); unfold L_Etat;
 auto with arith.
rewrite un_pas; rewrite H5; try rewrite H6; auto with arith.

intros; apply (Rec3' _ _ _ _ (make_trois_end (S t) x)); auto with arith.
...
apply GBA_dollarC; auto with arith.

intros; apply (GC_dollarB (S (S t)) x); auto with arith.
...
intros; apply rec_triangle_inf; auto with arith.
...
intros; elim (Hor_tr_inf t (S (S x)) cote); intros; split; auto with arith.
elim H0; intros; elim H4; auto with arith.
Qed.
...
elim (Ht1_bissect t x cote) with (dx := S n); auto with arith; intros.
rewrite <- plus_n_Sm; apply DD_hddollar; auto with arith.
do 2 rewrite <- plus_Snm_nSm; rewrite plus_n_Sm; auto with arith.

do 2 rewrite <- plus_Snm_nSm; do 2 rewrite plus_n_Sm; auto with arith.
Qed.
...
intros; apply (Ht1_DD t x (S haut)); auto with arith.
Qed.
...
rewrite H; rewrite H2; try rewrite H3; auto with arith.
Qed.

Remark G20 : Etat (S (S t)) 0 = G.
rewrite demi_pas; rewrite A10; rewrite C11; auto with arith.
Qed.

Remark B21 : Etat (S (S t)) 1 = B.
rewrite un_pas; rewrite A10; rewrite C11; auto with arith.
Qed.
...
generalize (Ht0_bissect 0); rewrite plus_un; intros H; elim H; auto with arith.

generalize (Ht0_bissect 1); rewrite plus_un; intros H; elim H; auto with arith.
...
generalize (Ht0_DD (pred (pred long))); rewrite plus_Snm_nSm;
 repeat rewrite <- S_pred; auto with arith.
Qed.
